...
function tarotCard(){
    ...
    card.className = "card"
    card.style.animation = "cardAnimation 7s ease-in-out infinite"
...
for(let i=0; i<22;i++){
    ...
    card.style.animationDelay = `${-randomNum*(7/22)}s`
    let cardTranslationX = 0
    let cardTranslationY = 0
    cardTranslationX += 280*Math.cos(degree(220+i*100/22))
    card.children[1].children[0].style.backgroundColor=`rgb(${i*(255/22)},${i*(255/22)},${i*(255/22)})`
    cardTranslationY += 40*Math.sin(degree(i*180/22))
    let yOriginal = cardTranslationY
    let xOriginal = cardTranslationX
    let rotateAngle = 30-i*(60/22)
    setTimeout(apply, 10)
    function apply(){
    ...
    setTimeout(()=>{
        card.children[1].style.backgroundColor = `rgb(${i*(255/22)},${i*(255/22)},${i*(255/22)})`
    ...
    const conditionChecker = setInterval(() => {
        if (chose === true) {
          clearInterval(conditionChecker); 
          finishChoose();   
        }
      }, 100);
