...
let overView = document.getElementById("analysis")
...
function speech(text){
    setTimeout(()=>{
        const speechContent = new SpeechSynthesisUtterance(text);
        const voices = speechSynthesis.getVoices()
        speechContent.voice = voices[101]
        speechSynthesis.speak(speechContent);
        },1000)
}
                        ...
                        messages: [
                            {
                                ...
                                content:
                                `
                                You are a tarot reader. When the user asks a question, 
                                respond in 1 short sentences with your immediate thoughts and feelings about it.
                                 Share your impressions and any personal (even fictional) experiences that come to mind.
                                 Don't answer the queston yet as this is only the reacton before actually performing the reading
                                  Keep the tone insightful and authentic.`
                            },
                            {
                                role: 'user',
...
function chooseStart(){
    setTimeout(()=>{
        questionPage.style.transform = "translate(0px,300px)"
...
function startGo(){
    ...
    chooseState = false
    reading
    center.removeEventListener("mouseenter", central)
    center.removeEventListener("mouseleave", decentral)
    center.removeEventListener("click", showCentral)
...
for(let i=0; i<22;i++){
    ...
    guide.innerHTML = "Hi there! How can I help you?"
    input.innerHTML = ""
    ...
    center.addEventListener("click", showCentral)

    function showCentral(event){
        if(revealNum === 4){
        showAnalysis()
        event.stopPropagation()
        }
